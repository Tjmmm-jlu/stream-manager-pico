...
{
    // Recovery revision 28: build with the PICO GLES warmup guard.
    ...
    private static void RunRequestedFinalBuild()
    {
        ...
        string request = Path.Combine(
            projectRoot,
            FinalRequestPath.Replace('/', Path.DirectorySeparatorChar));
        if (!File.Exists(request)) return;
    ...
    public static void BuildFinalAStageApk()
    {
        BuildSceneApk(
            ...
            FinalOutputPath,
            FinalReportPath,
            // Never reuse PlayerDataCache for this scene. A stale cached level0
            // can make the Android player abort with "Position out of bounds"
            // even though Unity reports the APK build itself as successful.
            BuildOptions.CleanBuildCache,
            "full A-stage player rebuild with PlayerDataCache cleared (Scripts Only disabled)");
    }
